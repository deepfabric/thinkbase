...
func FuncEval(op int, args []value.Value) (value.Value, error) {
	if os, ok := FuncOps[op]; ok {
		for _, o := range os {
	...
	return value.ConstNull, fmt.Errorf("%s not yet implemented for %s", OpName[op], value.Array(args))
}
		...
		&FuncOp{
			...
			Fn: func(args []value.Value) (value.Value, error) {
				...
				for _, arg := range args {
					if oid := arg.ResolvedType().Oid; oid == types.T_null || oid == types.T_table {
						continue
					}
